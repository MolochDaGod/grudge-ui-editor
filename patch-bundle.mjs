 ...
 * Idempotent — skips patches already applied.
...
replace(mpInsertPoint, mpComponent + mpInsertPoint, 'main panel component wk');
...
if (patches.length) fs.writeFileSync(BUNDLE, s);
if (patches.length) console.log('Applied patches:', patches.join(', '));
if (skipped.length) console.log('Already applied:', skipped.join(', '));
if (!patches.length && !skipped.length) console.log('No patches matched.');
if (!process.exitCode) console.log('Bundle OK');